            ...
            @if($reservations)
                @foreach($reservations as $reservation)
                    ...
                    <div style="display: flex; align-items: center; border: 1px solid #ccc; padding: 10px; background: rgb(0, 89, 255); color: white;">
                        ...
                        <!-- 予約変更フォーム -->
                        <form class="change-reservation-form" data-reservation-id="{{ $reservation->id }}" action="{{ route('reservations.update', ['reservationId' => $reservation->id]) }}" method="post" style="display: none;">
                            @csrf
                            @method('PUT')
...
<!-- お気に入り店舗の表示 -->
<div class="col-md-6" style="width: 50%; box-sizing: border-box; min-height: 100px; overflow-y: auto;">
    ...
    @if($favoriteShops->isNotEmpty())
        <div class="shop-list" style="display: flex; flex-wrap: wrap;">
            @foreach($favoriteShops as $favoriteShop)
                <div class="shop-item" id="favorite-{{ $favoriteShop->id }}" style="width: 48%; margin-right: 2%; margin-bottom: 20px;">
                    @php
                        $shop = $favoriteShop->shop->first();
                    @endphp
                    @if($shop)
                        @if($shop->image_url)
                            <img src="{{ $shop->image_url }}" alt="{{ $shop->name }}" style="width: 100%;">
                        @else
                            <p>画像なし</p>
                        @endif
